# 更新你的 iPhone 来解决一个大的 FaceTime 问题

> 原文：<https://lifehacker.com/update-your-iphone-to-solve-a-big-facetime-problem-1842753010>

一个意想不到的 iOS/iPadOS 更新今天发布，虽然它没有增加任何新功能，但它*确实*修复了一个令人讨厌的错误，该错误阻止了 iOS/iPadOS 13.4 的一些用户与运行 iOS 9.3.1、macOS 10.11.16 或其他旧操作系统的设备上的人进行 FaceTiming。鉴于如今我们比以往任何时候都更加依赖 [视频聊天](https://lifehacker.com/tag/video-chat)，这种错误比通常情况下更加令人恼火——尤其是如果你试图使用老式智能手机或电脑与亲戚保持联系。

Watch

补丁说明中没有提到这个错误的原因，但不管是什么原因，iOS/iPadOS 13.4.1 似乎已经修复了它。如果你在旧的苹果设备上与人交流有困难，一定要尽快更新你的 iPhone 和/或 iPad。

除了 FaceTime 调用修复之外，更新还修补了一个使通过快速动作菜单启用蓝牙变得困难的错误。如果你有 iPad Pro，还有一个额外的修正:

*“解决了 iPad Pro 12.9 英寸(第四代)和 iPad Pro 11 英寸(第二代)上的一个问题，在控制中心或锁定屏幕上点击手电筒按钮后，手电筒可能无法打开”*

这是 u pdate 中调整的完整列表——就像我说的，它非常稀疏，但是如果你因为 FaceTime 问题而推迟安装 iOS/iPadOS 13.4，你现在可以安全地更新。为了这样做:

1.  进入**设置>通用>软件更新。**
2.  点击**“下载并安装”**
3.  等待更新完成，你就准备好了。