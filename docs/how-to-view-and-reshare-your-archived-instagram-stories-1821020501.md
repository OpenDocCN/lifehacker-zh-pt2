# 如何查看和分享您的 Instagram 存档故事

> 原文：<https://lifehacker.com/how-to-view-and-reshare-your-archived-instagram-stories-1821020501>

你的 Instagram 故事会在 24 小时后从你的 feed 中消失，但它们不会永远消失:它们实际上保存在你个人资料的私人部分，你可以随时返回查看或重新分享它们。下面是怎么做的。



点击 Instagram 应用程序上的个人资料页面，点击屏幕右上角的三条垂直线，然后从弹出菜单中选择存档。在这里你可以查看存档的故事(和 [存档的文章](https://www.lifehacker.com.au/2017/06/now-you-can-hide-your-terrible-instagram-photos-without-deleting-them/)——只需点击屏幕顶部的“存档”即可在两者之间切换)。

你可以点击你档案中的任何故事来观看。

*   要将其添加到您当前的故事中，请点击**分享**。
*   要将其添加到您的个人资料页面上的突出显示(保持公开)，请点击**突出显示**并选择一个现有的突出显示，将其与分组或创建一个新的突出显示。
*   要以静态帖子的形式分享故事，请点击**更多>分享为帖子**。在这个菜单上，你还可以将故事保存为照片或永久删除。

Instagram 会自动存档你的所有故事(私下)，除非你禁用该功能。为此，请轻按归档屏幕右上角的三个点，然后点按底部出现的弹出式菜单中的“设置”。将保存到档案按钮切换到关闭位置。

*这个故事最初由艾米丽·普莱斯(Emily Price)于 2017 年发表，由艾米丽·龙(Emily Long)于 2020 年 4 月 29 日更新。我们修改了步骤以匹配 Instagram 内的更新，重写了第一段，并添加了新的截图。*