...
您可以通过将 AirPod 与 iPhone 配对并将其中一个(或两个)放入耳朵来确认它的固件编号。从那里，去**设置>一般>左右。**向下滚动并点击 AirPods 的名称，然后查找设备的**固件版本**如果是 2D3，那么你的设备运行的是未发布的固件。

你不能回滚 AirPods 固件上的固件，所以你需要等待苹果在未来的某个时候最终发布更新。尽管有广泛的报道，苹果还没有公开评论不均衡的固件，但据报道，苹果支持部门证实固件更新正在进行中。谁也说不准这一更新什么时候会最终推出，所以你唯一的选择是等待——并习惯一些单耳听。